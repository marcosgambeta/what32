...
HB_FUNC(RGB)
{
  hb_retnl((ULONG)(COLORREF)(((BYTE)(hb_parni(1)) | ((WORD)((BYTE)(hb_parni(2))) << 8)) |
...
HB_FUNC(GETGVALUE)
{
  hb_retni((INT)GetGValue(w32_par_DWORD(1)));
}
...
HB_FUNC(GETBVALUE)
{
  hb_retni((INT)GetBValue(w32_par_DWORD(1)));
}
...
HB_FUNC(GETRVALUE)
{
  hb_retni((INT)GetRValue(w32_par_DWORD(1)));
}
...
//-----------------------------------------------------------------------------
// WINUSERAPI BOOL WINAPI DrawAnimatedRects( IN HWND hwnd, IN int idAni, IN CONST RECT * lprcFrom, IN CONST RECT *
// lprcTo);
