...
// WINUSERAPI HICON WINAPI CreateIcon( IN HINSTANCE hInstance, IN int nWidth, IN int nHeight, IN BYTE cPlanes, IN BYTE
// cBitsPixel, IN CONST BYTE *lpbANDbits, IN CONST BYTE *lpbXORbits);

HB_FUNC(CREATEICON)
{

  hb_retnl((LONG)CreateIcon(ISNIL(1) ? GetModuleHandle(NULL) : (HINSTANCE)hb_parnl(1), hb_parni(2), hb_parni(3),
                            (BYTE)hb_parni(4), (BYTE)hb_parni(5), (BYTE *)hb_parcx(5), (BYTE *)hb_parcx(6)));
}
...
//-----------------------------------------------------------------------------
// WINUSERAPI int WINAPI LookupIconIdFromDirectoryEx( IN PBYTE presbits, IN BOOL fIcon, IN int cxDesired, IN int
// cyDesired, IN UINT Flags);
...
//-----------------------------------------------------------------------------
// WINUSERAPI HICON WINAPI CreateIconFromResource( IN PBYTE presbits, IN DWORD dwResSize, IN BOOL fIcon, IN DWORD
// dwVer);
...
//-----------------------------------------------------------------------------
// WINUSERAPI HICON WINAPI CreateIconFromResourceEx( IN PBYTE presbits, IN DWORD dwResSize, IN BOOL fIcon, IN DWORD
// dwVer, IN int cxDesired, IN int cyDesired, IN UINT Flags);
...
HB_FUNC(LOADIMAGE)
{
  hb_retnl((LONG)LoadImage(ISNIL(1) ? GetModuleHandle(NULL) : (HINSTANCE)hb_parnl(1), (LPCSTR)hb_parcx(2),
...
//-----------------------------------------------------------------------------
// WINUSERAPI BOOL WINAPI DrawIconEx( IN HDC hdc, IN int xLeft, IN int yTop, IN HICON hIcon, IN int cxWidth, IN int
// cyWidth, IN UINT istepIfAniCur, IN HBRUSH hbrFlickerFreeDraw, IN UINT diFlags);
...
{
  hb_retnl((LONG)DuplicateIcon(ISNIL(1) ? GetModuleHandle(NULL) : (HINSTANCE)hb_parnl(1), (HICON)hb_parnl(2)));
}
#endif
//-----------------------------------------------------------------------------
...
HB_FUNC(EXTRACTASSOCIATEDICON)
{
  ...
  hiRet =
      ExtractAssociatedIcon((ISNIL(1) ? GetModuleHandle(NULL) : (HINSTANCE)hb_parnl(1)), (LPSTR)hb_parcx(2), &lpiIcon);
...
HB_FUNC(EXTRACTICON)
{
  hb_retnl((LONG)ExtractIcon(ISNIL(1) ? GetModuleHandle(NULL) : (HINSTANCE)hb_parnl(1), (LPCSTR)hb_parcx(2),
                             (UINT)hb_parni(3)));
}
