...
{
  hb_retni(GetMenuItemID((HMENU)hb_parnl(1), // handle to menu
...
{
  MENUTEMPLATE *mt = (MENUTEMPLATE *)hb_param(1, HB_IT_STRING)->item.asString.value;

  hb_retnl((LONG)LoadMenuIndirect(mt));
}

//-----------------------------------------------------------------------------
// WINUSERAPI BOOL WINAPI ChangeMenuA( IN HMENU hMenu, IN UINT cmd, IN LPCSTR lpszNewItem, IN UINT cmdInsert, IN UINT
// flags);

HB_FUNC(CHANGEMENU)
{
  hb_retl(ChangeMenu((HMENU)hb_parnl(1), (UINT)hb_parni(2), (LPCSTR)hb_parcx(3), (UINT)hb_parni(4), (UINT)hb_parni(5)));
}
...
HB_FUNC(REMOVEMENU)
{
  hb_retl(RemoveMenu((HMENU)hb_parnl(1), (UINT)hb_parni(2), (UINT)hb_parni(3)));
}
...
//-----------------------------------------------------------------------------
// WINUSERAPI BOOL WINAPI ModifyMenuA( IN HMENU hMnu, IN UINT uPosition, IN UINT uFlags, IN UINT_PTR uIDNewItem, IN
// LPCSTR lpNewItem );
...
HB_FUNC(INSERTMENUITEM)
{
  ...
  hb_retl(InsertMenuItem((HMENU)hb_parnl(1), (UINT)hb_parni(2), hb_parl(3), lpcmenuitemInfoa));
}
...
{
  LPCMENUITEMINFOA lpcmenuitemInfoa = (LPCMENUITEMINFOA)hb_param(4, HB_IT_STRING)->item.asString.value;
  hb_retl(SetMenuItemInfo((HMENU)hb_parnl(1), (UINT)hb_parni(2), hb_parl(3), lpcmenuitemInfoa));
}
...
{
  hb_retl(CheckMenuRadioItem((HMENU)hb_parnl(1), (UINT)hb_parni(2), (UINT)hb_parni(3), (UINT)hb_parni(4),
                             (UINT)hb_parni(5)));
}
